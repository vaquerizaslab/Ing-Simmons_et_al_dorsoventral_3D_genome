...
output_folder = "figures/figure_3_panels/"

logging.info("Working on sample %s", str(sample_name))

# set up data

hic_10kb_file = os.path.join("data", "hic", "merged", sample_name, "hic",
                             sample_name + "_10kb.hic")
hic_10kb = fanc.load(hic_10kb_file)

hic_2kb_file = os.path.join("data", "hic", "merged", sample_name, "hic",
                            sample_name + "_2kb.hic")
hic_2kb = fanc.load(hic_2kb_file)

# compartments data
hic_50kb_file = os.path.join("data", "hic", "merged", sample_name, "hic",
                             sample_name + "_50kb_masked.hic")

hic_50kb = fanc.load(hic_50kb_file)
ab_file = os.path.join("data", "hic", "merged", sample_name, "hic",
...
if not os.path.exists(tad_am_output_file):
    ...
    tad_am = fanc.AggregateMatrix.from_regions(hic_2kb, tads.regions, log=True, oe=True,
...
if not os.path.exists(loop_am_output_file):
    ...
    loop_am = fanc.AggregateMatrix.from_center_pairs(hic_2kb, loops, log=True, oe=True,
...
p_hic_small = fancplot.HicPlot2D(hic_2kb, norm='log', vmin=1e-04, vmax=1e-01,
